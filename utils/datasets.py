...
class LoadImages:  # for inference_utils
...
class LoadWebcam:  # for inference_utils
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        s = np.stack([letterbox(x, new_shape=self.img_size)[0].shape for x in self.imgs], 0)  # inference_utils shapes
        self.rect = np.unique(s, axis=0).shape[0] == 1  # rect inference_utils if all shapes equal
        if not self.rect:
